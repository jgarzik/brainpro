...
pub fn schema(opts: &SchemaOptions) -> Value {
    if opts.optimize {
        ...
        json!({
            ...
            "function": {
                "name": "Edit",
                "description": "Edit file with find/replace. Requires permission. MUST Read the file first before editing. The find string must match exactly (including whitespace/indentation). Each find must be unique in the file to avoid wrong-match edits. Preserve exact indentation in both find and replace. Use count:0 to replace all occurrences (e.g., variable rename). Multiple edits in one call are applied sequentially.",
                "parameters": {
                    "type": "object",
                    "properties": {
